...
export function ChatList({ onChatSelect, onCreateGroup, onSearchUsers, selectedChatId, refreshTrigger }: ChatListProps) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (message: {
      ...
      fetchChats(false); // Don't show loading for message updates
    };
    ...
    const handleChatListUpdate = (data: {
      ...
      // Refresh the chat list to show updated last message
      fetchChats(false); // Don't show loading for chat list updates
    };

    const handleGlobalChatListUpdate = (data: {
      chatId: string;
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <p className="font-bold text-sm truncate text-accent-foreground">
                              {searchQuery ? highlightText(getChatDisplayName(chat), searchQuery) : getChatDisplayName(chat)}
                            </p>
                          ...
                          <div className="flex items-center justify-between mt-1">
                            <p className="text-xs text-muted-foreground truncate">
                              {searchQuery ? highlightText(getLastMessagePreview(chat), searchQuery) : getLastMessagePreview(chat)}
                            </p>
